...
import { touristNationalityMapper } from './mappers/touristNationalityToDomain'
  ...
  processFirstDataset(dataset: Dataset): TouristSpending[] {
    ...
    for (const data of datasetPrimitives.data) {
      ...
      if (!touristSpending) {
        ...
        if (!residencePlace) {
          throw new Error(
            `Residence place for nationality ${residencePlacesLabels[indexPlace]} not found`
          )
        }
  ...
  processSecondDataset(dataset: Dataset, storageDataProcessed: TouristSpending[]) {
    ...
    for (const data of datasetPrimitives.data) {
      ...
      if (
        ...
        const conceptLabelInDomain = touristSpendingConceptToDomain(conceptsLabels[indexCode])

        const spendingByConcept =
          touristSpending.getSpendingByConceptwithLabel(conceptLabelInDomain)

        if (!spendingByConcept) {
          throw new Error(`Spending concept for label ${conceptLabelInDomain} not found`)
        }
        ...
        const touristNationalityInDomain = touristNationalityMapper(
          data.categoryCodes[1] as TouristNationalityResponse
        )

        const residencePlace = touristSpending.getResidencePlacesByLabel(touristNationalityInDomain)

        if (!residencePlace) {
          throw new Error(`Residence place for nationality ${touristNationalityInDomain} not found`)
        }

        const indexCode = conceptsCodes.indexOf(data.categoryCodes[0])

        const residencePlaceConcept = residencePlace.getConceptWithLabel(
          touristSpendingConceptToDomain(conceptsLabels[indexCode])
        )
